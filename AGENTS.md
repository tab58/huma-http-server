...
| `lib/jwt/` | `jwt` | HS256 JWT create/verify. `AccessToken`/`RefreshToken` string types, 15 min / 7 day expiries, `TokenGenerator` interface + impl, refresh-token exchange. Claims are flat `map[string]string` plus `exp`/`iat`. Token type is inferred from `exp - iat` duration (±1 s), not a claim. Only tested package (`jwt_test.go`). |
